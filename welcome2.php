...
require("connection.php");
  ...
  $questions = array(
    array("Which Disney character famously leaves a glass slipper behind at a royal ball?", "Cinderella", array("Pocahontas", "Sleeping Beauty", "Cinderella", "Elsa")),
    array("Where would a peruke be worn?", "On the head", array("Around the neck", "On the head", "Around the waist", "On the wrist")),
    array("What is the largest active volcano in the world?", "Mount Loa", array("Mount Etna", "Mount Vesuvius", "Mount Loa", "Mount Batur")),
    array("In which museum can you find Leonardo Da Vinci’s Mona Lisa?", "Le Louvre", array("Le Louvre", "Uffizi Museum", "British Museum", "Metropolitan Museum of Art"))
  );
  ...
  // Retrieve user's wallet balance from database
...
if (isset($_POST['submit'])) {
  ...
  //$result = $conn->query($sql);
  //if ($result->num_rows > 0) {
    // User has already taken the quiz for the current day
  ...
  //  $error_message = "Error: You have already taken the quiz for today. Please try again tomorrow for the next quiz.";
   ...
   //exit();
  //} else {
    // User has not taken the quiz for the current day
  $error_messages = array(); // array to store error messages
  // validate question's answer
...
nav li a:hover {
  ...
  <ul>
    ...
    <h2 style="text-align: center; color: orange">Quizzle - Day 2</h2>
